...
file_in = '/mnt/lustre/nknyazeva/courseWork5/data/result/genes_GRCh38/ortho_MACMU18455_HUMAN23250/orf_HUMAN23250_align_mafft.afa'
file_out = '/mnt/lustre/nknyazeva/courseWork5/data/result/genes_GRCh38/ortho_MACMU18455_HUMAN23250/pn_ps_orf_HUMAN23250_align_mafft.afa'
        ...
        for name in names:
            ...
            with open(name, 'r') as handle:
                ...
                for codon_position in xrange(len(seq_izip)):
                    ...
                    if len(seq_izip[codon_position]) > 0 \
                            and all(codontable[x] == codontable[seq_izip[codon_position][0]] for x in seq_izip[codon_position]) \
                            and all(x in four_folded_triplets for x in seq_izip[codon_position]):
                        number_syn_sites += 1
                ...
                for codon_position in xrange(len(seq_izip)):
                    if len(seq_izip[codon_position]) > 1:
                        ...
                        if len(list(set(triplets))) > 1:
                            ...
                            counter_triplets = dict(Counter(triplets))
                            print(counter_triplets)
                            values = counter_triplets.values()

                            for value in values:
                                pn = pn - ((float(value)/len_triplets) ** 2)

                            lst_pn.append(pn)


                if number_nosyn_sites > 0 and number_syn_sites > 0: # try to avoid zero division
                    # print('lst_pn %s '  % lst_pn)
                    print('{}\t{}\t{}\n'.format(gene_name, float(sum(lst_pn))/number_nosyn_sites, float(sum(lst_ps))/number_syn_sites))

                    print('lst_pn %s' % len(lst_pn))
                    print('lst_ps %s' % len(lst_ps))
                    print('number_nosyn_sites %s' % number_nosyn_sites)
                    print('number_syn_sites %s' % number_syn_sites)

                    # print(gene_name)
                    # print(float(sum(lst_pn))/number_nosyn_sites)
                    # print(float(sum(lst_ps))/number_syn_sites)
