...
dir_sh = '/mnt/lustre/nknyazeva/courseWork5/scripts/vcf_to_fasta_sh/'
...
for i in range(71):
    ...
    for line in file_in:
        ...
        for org in range(2):
            file_out.write('for id in ' + haplotypes[org] + '\n'
                           'do ' + '\n')

            file_out.write('samtools faidx ' + refs[org] + ' ' + coords[org] +
                            ' | /home/nknyazeva/tools/bcftools-1.9/bin/bcftools consensus -s $id ' +
                            vcfs[org] + ' > /mnt/lustre/nknyazeva/courseWork5/data/result/genes/ortho_' +
                            ids[0] + '_' + ids[1] + '/' + ids[org] + '/consensus_homo_' + ids[org] + '_$id.fasta' + '\n')
